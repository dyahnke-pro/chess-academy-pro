WORK ORDER — Rewrite the banked Naroditsky video narrations for the FRENCH DEFENSE (our own words, DNA outline)
...
each narrated beat: kind = "main" | "branch" | "trap" | "review".
If in post-game his words don't match the `fen` (he's talking about another line),
set `reanchor: true` and keep `spoken` to what IS true on the board.
...
## Coordination
- Work on your OWN branch off main (e.g. `claude/narration-french`); open your own
  draft PR. You only write `data/video-narration-voiced/<your ids>.json` — no
  conflict with other sessions.
- VERIFY each video's real opening FROM ITS MOVES before authoring (titles lie).
  If a listed video's GAME isn't a French, set `openingName` to the real opening
  and note it; don't force it.

## YOUR VIDEOS (French Defense, 13)
6si_sctYAS0 8O4UG9NtUoM 9XUJ2GmQ2CU FozbGMUOOjs J4WzTSR3hmo JKxlT73xpYo
QUk_oflYX0M Qkv7D-BGAt4 WSEAjQgHpYs bRMHNmDMLqw hw9tEjYabd8 ngKFZPlhP2c
tWGrKGoNNEA
(WSEAjQgHpYs is a multi-opening title — file by the actual game.)
