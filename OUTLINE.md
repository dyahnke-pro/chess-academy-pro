...
- 🟠 2. The one SW check a single deploy cannot make — **INSTRUMENT BUILT, and the board line was wrong about when it can run** (2026-09-21). `scripts/audit-sw-two-deploy-prod.mjs` holds a live session on deploy N, waits for N+1 to land underneath it, and asserts the page was not taken over, no HASHED asset 404s across two driven lazy navigations, and the session still works. Rows are ungraded-not-green when no deploy lands. 🔴 **"Rides along with whatever ships next" is FALSE: it must ride a deploy that changes the BUNDLE.** A docs/scripts/test push deploys and can emit a byte-identical entry chunk, so the browser is never offered a new worker and there is no handover to observe — proven tonight on a five-file push where `git diff --name-only | grep ^src/ | grep -v .test.` returned nothing. Three signals to watch, in order of directness: `/sw.js` md5 (the precache manifest carries per-entry revisions), the `x-vercel-id` deployment id, then any `/assets/*` hash. Baseline captured at `index-CL9P6Cc6.js` / sw.js md5 `5d5bc0fc…` — **whether sw.js moves on a bundle-identical deploy is itself an open question the board does not have an answer to**, and it decides what a docs push costs a live session
