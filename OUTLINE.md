...
- 🔴 **`note-anchors.json` is STALE and its generator was BROKEN** (found 2026-09-20 while clearing 11e). `scripts/derive-note-anchors.mjs` hand-wrote a corpus list that the 2026-09-19 split invalidated — it pointed at `src/data/chessbrah-teachings.json`, which moved to `public/`, so the next real run would have thrown ENOENT. `noteAnchorDerivation.test` carried the SAME copy and died on ENOENT before a single assertion ran: a gate that could not run at all, which is why nobody noticed. Both now DERIVE from `corpora.json`, and the test resolves by CORPUS rather than by filename so a relocation cannot blind it again. The dry run derives **1118 anchors against the committed 1031** — the sidecar is stale by 87. NOT regenerated here: `note-anchors.json` feeds note SELECTION at runtime, so it needs its own commit and its own verification, not a ride-along in a type-fix batch.
- 🟠 **THE TWO GATES DISAGREE ABOUT THE SAME EXPRESSION** (found 2026-09-20 clearing 11e). `tsc -p tsconfig.tests.json` demands a cast where `npm run lint` calls that same cast an ERROR (`no-unnecessary-type-assertion`, and lint runs `--report-unused-disable-directives`, so it is an error not a warning). PROVEN by experiment on one expression: without the cast tsc reports 3 errors and eslint is clean; with it, the reverse. Both configs extend `tsconfig.app.json`, so the cause is not an obvious include difference and is NOT yet diagnosed. Consequence: the backlog cannot be driven to zero with assertions — every fix must change the TYPE (a typed mock, a generic parameter, a `?? ''` default) so neither gate has an assertion to object to. That is a better fix anyway, but it is forced rather than chosen, and the underlying disagreement is still open.
- 🟠 11e the test-type-error ceiling: **236 → 37** (2026-09-20), ceiling lowered in ship-check. Two systematic shapes fixed at the root — a `buildMoveResult` factory for the 15 sites still writing the pre-`pgn`/`history`/`moveNumber`/`turn` shape, and `capabilityOrigin` added to the Learn-surface mounts (it is REQUIRED by design, so the tests were stale, not the type). Then `bestMoveEval` (required, nullable — null is the honest default: "the engine reported none" is a fact, a number would be a claim) and `AnalysisLine`'s `rank`/`mate`. Then `OpeningVariation.annotations`, which is not a field AT ALL and never was — those fixtures were written against a shape the type has never had, while omitting the required `explanation`. 214/214 affected tests still pass
...
- 🟠 11l **does the live-commentary path fire on Learn at all?** MEASURED, first run of the new instrument: a real 5-ply Learn game emitted 4 `coach-decision` rows and **every one was `walk`** — zero `interrupt`. Not a defect on its face: both interrupt doors are gated on a CACHED engine analysis (`useLiveCoach`, `usePhaseNarration`), which a short game never warms. But the 2026-08-05 rule says Learn is "a game the coach TALKS YOU THROUGH", and if the running commentary needs a warm cache it may be rarer than intended. Answer it with a LONGER probe that waits for the cache, not with an assertion
...
- 🟠 11e the test-type-error ceiling 236 → **37** — hygiene, test files only; see §6 for what was fixed at the root.
