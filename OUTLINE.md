...
- 🟠 **WHY the entry hash moves on a bundle-identical push — TRACED, and the control may not exist** (2026-09-21). Diffed two consecutive builds: they differ from byte 70, inside `__vite__mapDeps`, where the entry embeds its dependency FILENAMES — `assets/web-BITZqWmZ.js` vs `assets/web-7Ov3xJEz.js`. So a downstream chunk's hash changed and the entry, which names it, changed with it. That is a CASCADE, not a timestamp: the question moves one level down to why `web-*.js` (the CapacitorUpdater/OTA chunk) differs across a docs-only push. Worth one look before spending four more deploys hunting a negative control — if something non-deterministic feeds that chunk, no bundle-identical push will ever fail to move the entry, and the absence of a counterexample is the finding rather than a gap in it
