...
- 🔴 **LEARN AUDIT ROW C — the concept invariant does not speak, and the cause is NOT yet established** (prod, 2026-09-21, 14/15 twice on `ce49648`). The driven game stops at **ply 4**, one short of 3.Nc3 attacking the queen, where three earlier runs today reached ply 5 and passed this row on "Careful — your queen on d5 is attacked". REPRODUCIBLE — my first reading called it cold-bundle variance and that was WRONG, corrected here rather than left standing. What IS established: the computers are fine (85 tests green across `conceptEngine`, `tacticTypeUnification`, `useLiveCoach`, `usePhaseNarration`), so this is the live DRIVER not reaching the position, not a broken beat. No evidence ties it to the corpus cut — that beat is code-computed, not corpus-fed — and no claim either way is made here. TO FIX: read why the driver stops a move early before touching any product code
