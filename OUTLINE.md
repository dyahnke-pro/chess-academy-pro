...
- 🟠 11e the test-type-error ceiling: **236 → 197** (2026-09-20), ceiling lowered in ship-check. Two systematic shapes fixed at the root — a `buildMoveResult` factory for the 15 sites still writing the pre-`pgn`/`history`/`moveNumber`/`turn` shape, and `capabilityOrigin` added to the Learn-surface mounts (it is REQUIRED by design, so the tests were stale, not the type). Then `bestMoveEval` (required, nullable — null is the honest default: "the engine reported none" is a fact, a number would be a claim) and `AnalysisLine`'s `rank`/`mate`. 188/188 affected tests still pass
...
- 🟠 11e the test-type-error ceiling 236 → **197** — hygiene, test files only; see §6 for what was fixed at the root.
