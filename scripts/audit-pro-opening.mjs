...
const ROUTE = process.env.AUDIT_ROUTE ?? '/openings/pro/ericrosen/pro-ericrosen-stafford';
...
async function gotoStafford(page) {
  ...
  for (let i = 0; i < 25; i++) {
    ...
    if (new RegExp(process.env.AUDIT_NAME ?? 'Stafford','i').test(t) && !/Loading opening/i.test(t)) return true;
    ...
    r.onsuccess = () => {
      ...
      function doUnlock() {
        try {
          ...
          const g = st2.get((process.env.AUDIT_OID ?? 'pro-ericrosen-stafford'));
...
const main = async () => {
  ...
  try {
    ...
    rec('Main line tab present', /Main line/.test(body0));
    ...
    if (await watch.count() > 0) {
      ...
      const speaksStafford = new RegExp(process.env.AUDIT_NAME ?? 'Stafford','i').test(onscreen) || tts.some((t) => new RegExp(process.env.AUDIT_NAME ?? 'Stafford','i').test(t));
