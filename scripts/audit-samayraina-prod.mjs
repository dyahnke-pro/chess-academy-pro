// Post-deploy audit for the FULL Samay Raina pro-rep (G1 + G9.3 Gate A), live
// PROD. Data-driven over every pro-samayraina-* opening in pro-repertoires.json:
...
const OPENINGS = rep.openings.filter((o) => o.id.startsWith('pro-samayraina-')).map((o) => o.id);
...
console.log(`=== Samay Raina FULL pro-rep audit vs PROD (${PROD}) — ${OPENINGS.length} openings ===\n`);
...
for (const id of OPENINGS) {
  await page.goto(`${PROD}/openings/pro/samayraina/${id}`, { waitUntil: 'networkidle', timeout: 25000 }).catch(() => null);
