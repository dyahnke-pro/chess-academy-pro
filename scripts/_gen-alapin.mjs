// Generate the Sicilian Alapin masterclass .ts files from the validated
...
const lessons = JSON.parse(readFileSync('scripts/_alapin-content.json', 'utf-8'));
...
const mainBody = `export const SICILIAN_ALAPIN_LESSON: LessonScript = ${lessonObj(main)};\n`;
writeFileSync('src/data/lessons/sicilianAlapin.ts', `${header(mainBody)}\n${mainBody}`);
...
const varBody = `export const SICILIAN_ALAPIN_VARIATION_LESSONS: Record<string, LessonScript> = {\n${varEntries}\n};\n`;
writeFileSync('src/data/lessons/sicilianAlapinVariations.ts', `${header(varBody)}\n${varBody}`);

console.log(`generated: sicilianAlapin.ts (main, ${main.beats.length} beats), sicilianAlapinVariations.ts (${variations.length} variations)`);
