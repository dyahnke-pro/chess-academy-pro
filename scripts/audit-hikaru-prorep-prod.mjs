// INTERACTIVE 3-instrument post-deploy audit (G1 + G7 + G9.3) for the Hikaru
...
const OPENINGS = [
  { id: 'pro-hikaru-caro-kann', gems: 5 },
  { id: 'pro-hikaru-closed-sicilian', gems: 1 },
  { id: 'pro-hikaru-pirc-modern', gems: 0 },
  { id: 'pro-hikaru-reti', gems: 0 },
  { id: 'pro-hikaru-nimzo-larsen', gems: 0 },
];
...
console.log(`=== INTERACTIVE 3-instrument Hikaru pro-rep audit vs PROD (${PROD}) ===\n`);
...
if (await search.count() > 0) {
  for (const q of ['hikaru', 'nakamura', 'grand prix', 'nimzo larsen', 'reti']) { await search.fill(q).catch(() => null); await page.waitForTimeout(1000); }
