// Ingest the ENTIRE "Chess Strategy" (Capablanca, 1921, public domain) into
...
const SRC_URL = 'https://www.gutenberg.org/cache/epub/5614/pg5614-images.html';
const CACHE = 'data/sources/chess-strategy-1915.html';
const OUT = 'src/data/library/chess-strategy.json';
...
let start = 0;
  for (let i = 0; i < lines.length; i++) if (/RULES OF THE GAME/i.test(lines[i])) start = i;

let chapter = 'Chess Strategy';
const paras = [];
let buf = [];
let sawFig = false;
const flush = () => {
...
for (const p of paras) {
  if (cur.paras.length && cur.heading !== p.heading) {
    pages.push({ id: `cs-${pi++}`, heading: cur.heading, text: cur.paras.join('\n\n') });
...
console.log(`Ingested Chess Strategy -> ${OUT}`);
