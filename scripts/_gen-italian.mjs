...
const lessons = JSON.parse(readFileSync('scripts/_italian-content.json', 'utf-8'));
const COLOR = {
...
const mainBody = `export const ITALIAN_GAME_LESSON: LessonScript = ${lessonObj(main)};\n`;
writeFileSync('src/data/lessons/italianGame.ts', `${header(mainBody)}\n${mainBody}`);
...
const varBody = `export const ITALIAN_GAME_VARIATION_LESSONS: Record<string, LessonScript> = {\n${varEntries}\n};\n`;
writeFileSync('src/data/lessons/italianGameVariations.ts', `${header(varBody)}\n${varBody}`);
...
`;
writeFileSync('src/data/lessons/italianGameTrapLessons.ts', trapsFile);

console.log('generated: italianGame.ts, italianGameVariations.ts, italianGameTrapLessons.ts');
