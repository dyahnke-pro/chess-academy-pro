...
const OUT_PATH = join(OUT_DIR, 'best-counters-shallow.json');
...
function pickBest(data, stm) {
  ...
  return data.moves.map(m => {
    ...
    const score = (wins + 0.5*(m.draws||0)) / total;
    return { san: m.san, uci: m.uci, total, wins, draws: m.draws||0, losses: stm==='white'?(m.black||0):(m.white||0), winRate: wins/total, expectedScore: score };
  }).filter(Boolean).sort((a,b)=>b.expectedScore-a.expectedScore)[0] || null;
}
...
async function findBest(o) {
  ...
  // SIGNATURE DEPTH: ply 5-8 is where most openings have their identity
  // established (the white opening's defining 3rd or 4th move).
  // Walk DEEPER if signature lacks data; only fall back to shallow if all else fails.
  for (const plyMax of [5, 6, 7, 8, 10, 12, 4]) {
    ...
    return { pliesAtQuery: pliesPlayed, dbSource: source, tailFen: fen, sideToMove: stm, totalGamesAtPosition: total,
      bestResponse: { san: best.san, uci: best.uci, games: best.total, winsForResponder: best.wins, lossesForResponder: best.losses, draws: best.draws, winRate: best.winRate, expectedScore: best.expectedScore },
...
async function main() {
  ...
  console.log(`processing ${all.length} openings — SIGNATURE DEPTH (5-8 first)`);
  const results = [];
  for (let i = 0; i < all.length; i++) {
    const o = all[i];
    const r = await findBest(o);
    results.push({ openingId: o.id, source: o._s, openingName: o.name, openingColor: o.color, ...r });
    const tag = r.bestResponse ? `✓ ${r.bestResponse.san} (score ${(r.bestResponse.expectedScore*100).toFixed(1)}%, ${r.bestResponse.games}g @ply${r.pliesAtQuery})` : `✗ ${r.error}`;
