// Generate the Sicilian Sveshnikov masterclass .ts files from the validated
...
const lessons = JSON.parse(readFileSync('scripts/_svesh-content.json', 'utf-8'));
...
const mainBody = `export const SICILIAN_SVESHNIKOV_LESSON: LessonScript = ${lessonObj(main)};\n`;
writeFileSync('src/data/lessons/sicilianSveshnikov.ts', `${header(mainBody)}\n${mainBody}`);
...
const varBody = `export const SICILIAN_SVESHNIKOV_VARIATION_LESSONS: Record<string, LessonScript> = {\n${varEntries}\n};\n`;
writeFileSync('src/data/lessons/sicilianSveshnikovVariations.ts', `${header(varBody)}\n${varBody}`);

console.log(`generated: sicilianSveshnikov.ts (main, ${main.beats.length} beats), sicilianSveshnikovVariations.ts (${variations.length} variations)`);
