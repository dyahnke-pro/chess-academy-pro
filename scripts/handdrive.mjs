...
async function dump() {
  return page.evaluate(() => { const out = []; for (const el of document.querySelectorAll('[data-testid]')) { const r = el.getBoundingClientRect(); if (r.width === 0 || r.height === 0) continue; const t = el.getAttribute('data-testid'); const tag = el.tagName.toLowerCase(); if (tag === 'button' || /modal|popup|callout|prompt|card|toast|banner|consent|help|calib|continue|unlock|rung|play-mode|difficulty|why|show-the-line|watch|learn|practice|variation|tab|lesson|narration|eval|blunder|interception|review|walk|skip|next|hint|reveal|ask/i.test(t)) { const txt = (el.innerText || '').replace(/\s+/g, ' ').slice(0, 50); out.push(`${tag}[${t}]${el.disabled ? '(disabled)' : ''} "${txt}"`); } } return [...new Set(out)]; });
}
async function exec(c) {
  try {
    if (c.action === 'nav') { await page.goto(c.url, { waitUntil: 'domcontentloaded', timeout: 60000 }); await page.waitForTimeout(c.wait || 1500); return { url: page.url() }; }
    if (c.action === 'reload') { await page.reload({ waitUntil: 'domcontentloaded' }); await page.waitForTimeout(c.wait || 1500); return { url: page.url() }; }
    if (c.action === 'dump') { return { url: page.url(), buttons: await dump() }; }
    if (c.action === 'text') { return { testid: c.testid, text: await page.locator(`[data-testid="${c.testid}"]`).first().innerText({ timeout: c.timeout || 4000 }).catch(() => null) }; }
    if (c.action === 'html') { return { html: await page.evaluate((sel) => { const el = sel ? document.querySelector(sel) : document.body; return el ? el.innerText.replace(/\s+/g, ' ').slice(0, 1200) : null; }, c.sel || null) }; }
    ...
    if (c.action === 'clicktext') { const l = page.getByText(c.txt, { exact: !!c.exact }).first(); await l.scrollIntoViewIfNeeded({ timeout: 6000 }).catch(() => {}); await l.click({ force: !!c.force, timeout: c.timeout || 8000 }); await page.waitForTimeout(c.wait || 1500); return { clickedText: c.txt, url: page.url() }; }
    if (c.action === 'type') { const l = page.locator(`[data-testid="${c.testid}"]`).first(); await l.click({ timeout: 6000 }); await l.pressSequentially(c.text, { delay: 20 }); if (c.enter) await l.press('Enter'); await page.waitForTimeout(c.wait || 1500); return { typed: c.text }; }
    if (c.action === 'seed') { await page.evaluate(async ({ rating, secret }) => { try { localStorage.setItem('auditMoveHook', '1'); } catch {} const dbs = await indexedDB.databases(); const nm = (dbs.find((d) => /chess/i.test(d.name || '')) || {}).name; if (!nm) return; await new Promise((res) => { const q = indexedDB.open(nm); q.onsuccess = () => { const db = q.result; if (!db.objectStoreNames.contains('profiles')) return res(); const tx = db.transaction('profiles', 'readwrite'); const ps = tx.objectStore('profiles'); const g = ps.getAll(); g.onsuccess = () => { for (const p of g.result) { p.preferences = p.preferences || {}; p.preferences.voiceEnabled = true; p.preferences.coachNarration = 'full'; if (secret) p.preferences.auditStreamSecret = secret; p.rating = rating; p.currentRating = rating; ps.put(p); } }; tx.oncomplete = () => res(); }; q.onerror = () => res(); setTimeout(res, 5000); }); }, { rating: c.rating || 900, secret: c.secret || '' }); return { seeded: true }; }
    ...
    if (c.action === 'boardclick') { const from = page.locator(`[data-square="${c.from}"]`).first(); await from.click({ timeout: 5000 }); await page.waitForTimeout(200); const to = page.locator(`[data-square="${c.to}"]`).first(); await to.click({ timeout: 5000 }); await page.waitForTimeout(c.wait || 2000); return { boardMoved: `${c.from}${c.to}` }; }
