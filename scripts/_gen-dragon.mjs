// Generate the Sicilian Dragon masterclass .ts files from the validated
...
const lessons = JSON.parse(readFileSync('scripts/_dragon-content.json', 'utf-8'));
...
const mainBody = `export const SICILIAN_DRAGON_LESSON: LessonScript = ${lessonObj(main)};\n`;
writeFileSync('src/data/lessons/sicilianDragon.ts', `${header(mainBody)}\n${mainBody}`);
...
const varBody = `export const SICILIAN_DRAGON_VARIATION_LESSONS: Record<string, LessonScript> = {\n${varEntries}\n};\n`;
writeFileSync('src/data/lessons/sicilianDragonVariations.ts', `${header(varBody)}\n${varBody}`);

console.log(`generated: sicilianDragon.ts (main, ${main.beats.length} beats), sicilianDragonVariations.ts (${variations.length} variations)`);
