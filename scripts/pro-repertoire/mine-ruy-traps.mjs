#!/usr/bin/env node
// Mine Naroditsky's chess.com Ruy games for opponent blunders that
// he punished concretely. Output: candidate trap sequences for human
// review/inclusion in pro-repertoires.json.
//
// Method:
//   1. Filter games to Naroditsky-as-white wins in the Ruy (e4 c5 c3 prefix)
...
const PREFIX = ["e4","e5","Nf3","Nc6","Bb5"];
...
console.log(`Scanned ${totalGames} games; ${ruyGames} are his Ruy wins as White; found ${blundersFound} blunder positions; ${trapsByPattern.size} unique trap patterns.`);
console.log();
...
fs.writeFileSync(
  'data/sources/danielnaroditsky-ruy-trap-candidates.json',
  JSON.stringify(
...
console.log(`\nFull candidate list (occurring in ≥2 games) written to data/sources/danielnaroditsky-ruy-trap-candidates.json`);
