#!/usr/bin/env node
// Mine Naroditsky's chess.com Najdorf games for opponent blunders that
// he punished concretely. Output: candidate trap sequences for human
// review/inclusion in pro-repertoires.json.
//
// Method:
//   1. Filter games to Naroditsky-as-white wins in the Najdorf (e4 c5 c3 prefix)
...
console.log(`Scanned ${totalGames} games; ${najdorfGames} are his Najdorf wins as Black; found ${blundersFound} White-blunder positions; ${trapsByPattern.size} unique trap patterns.`);
console.log();
...
fs.writeFileSync(
  'data/sources/danielnaroditsky-najdorf-trap-candidates.json',
  JSON.stringify(
...
console.log(`\nFull candidate list (occurring in ≥2 games) written to data/sources/danielnaroditsky-najdorf-trap-candidates.json`);
