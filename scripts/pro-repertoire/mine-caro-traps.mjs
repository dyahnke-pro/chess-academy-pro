#!/usr/bin/env node
// Mine Naroditsky's chess.com KID games for opponent blunders that
// he punished concretely. Output: candidate trap sequences for human
// review/inclusion in pro-repertoires.json.
//
// Method:
//   1. Filter games to Naroditsky-as-white wins in the KID (e4 c5 c3 prefix)
...
console.log(`Scanned ${totalGames} games; ${caroGames} are his Caro-Kann wins as Black; found ${blundersFound} White-blunder positions; ${trapsByPattern.size} unique trap patterns.`);
console.log();
...
fs.writeFileSync(
  'data/sources/danielnaroditsky-caro-trap-candidates.json',
  JSON.stringify(
...
console.log(`\nFull candidate list (occurring in ≥2 games) written to data/sources/danielnaroditsky-caro-trap-candidates.json`);
