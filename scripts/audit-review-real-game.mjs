...
const run = async () => {
  ...
  const CARD_TESTIDS = ['discussion-prompt', 'discussion-reason-picker', 'review-find-shot-card', 'review-find-shot-reveal', 'review-rewind-card', 'review-turning-point-card', 'review-turning-point-reveal', 'review-eval-card', 'review-eval-reveal', 'review-cameo-ask', 'review-cameo-playback', 'review-theory-ask', 'review-theory-playback', 'review-principle-quiz', 'review-sequence-ask', 'review-sequence-playback', 'review-capture-teach', 'review-capture-continue'];
  let evalCardFired = false;
  ...
  for (let step = 1; step <= 60; step++) {
    ...
    if ((await visibleCards()).length) {
      ...
      // §4 guess-the-eval: pick a bucket, then dismiss the reveal (like a human).
      if (await has(page, '[data-testid="review-eval-card"]')) { evalCardFired = true; await page.locator('[data-testid^="review-eval-pick-"]').first().click({ timeout: 1500 }).catch(() => {}); await page.waitForTimeout(700); if (await has(page, '[data-testid="review-eval-done"]')) { await page.locator('[data-testid="review-eval-done"]').first().click({ timeout: 1500 }).catch(() => {}); await page.waitForTimeout(300); } }
  ...
  // EVAL — §4 guess-the-eval fired once at a quiet middlegame position and was
  // answerable (a bucket pick + reveal). Proves the new diagnostic wired end-to-end.
  add('EVAL guess-the-eval', evalCardFired, evalCardFired ? 'the guess-the-eval card fired + was answered' : 'guess-the-eval never surfaced (window/collision gating too tight for this game)');
