...
**Status:** Foundational. All future Chess Academy Pro WOs build TO this vision.
...
## Migration debt (surfaces that will route through `coachService` in Phase 5)

These are existing per-surface LLM call sites that work today but bypass the unified spine. Each one is a Phase 5 (BRAIN-05) line item.

| Surface | Site | Current prompt | Notes |
|---|---|---|---|
| Blunder alert | `CoachGamePage.tsx` (BLUNDER INTERCEPTION block) — `getCoachChatResponse(..., BLUNDER_ALERT_ADDITION, ..., 'fast')` | `BLUNDER_ALERT_ADDITION` in `coachPrompts.ts` | Wired by WO-BLUNDER-COACH-READ (PR #319). Memory write already goes through `useCoachMemoryStore.appendConversationMessage` (BRAIN-aligned). Migrate the LLM call itself in BRAIN-05. |
| Game chat | `GameChatPanel` / `useGameChat` | per-surface system prompt | Migrate first (BRAIN-02). |
| Home / dashboard chat | `SmartSearchBar` / home chat handler | per-surface system prompt | BRAIN-03. |
| Move selector | `CoachGamePage` move-selection branch | per-surface logic + `requestedOpeningMoves` state | BRAIN-04. |
| Phase narration | `usePhaseNarration` | `PHASE_NARRATION_ADDITION` | BRAIN-05. |
| Position narration ("Read this position") | `usePositionNarration` | `POSITION_NARRATION_ADDITION` | BRAIN-05. |
| Hint tiers | `useHintSystem` | `HINT_TIER_*` prompts | BRAIN-05. |
| Walk-the-game review | `useReviewPlayback` / review segment generator | review prompts | BRAIN-05. |

Add new rows here as future per-surface LLM calls land. Every row is a BRAIN-05 task.
