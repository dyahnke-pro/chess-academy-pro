...
- **Progress:** Najdorf `6f3` was a true transposition-DUP of `main` (same idea+line) → merged 2026-06-25 (deleted, floor 9→8, note folded into `main`). **8 pairs remain — all genuine re-derivations** (different declared breaks, collapsed lines).
...
### Phase 4 — Variation explanation ≠ line  `[in progress]`
**Done (9 explanations rewritten to match their lines):** Vienna Stanley + Gambit; Reti KIA/Accepted/Gambit; Najdorf English Attack; Sveshnikov Kalashnikov; Budapest Alekhine; London-vs-Grünfeld. **+ Italian Main-Line-Nc3 LINE fixed** (was ending on the Ba3-dxc4 hung-bishop blunder → sound Bd3).
**Line-soundness CONFIRMED clean** via `scripts/content-audit/detect-line-material.cjs` (`_line-material.json`): the 13 lines ending student-down-material are all intentional gambits (Muzio/Moller/Belgrade/Cochrane/…) or mid-exchange/sharp-theory termini — the Italian was the lone genuine line-blunder. Two sharp lines (KID Saemisch sac, Vaganian poisoned-pawn) flagged for an engine/soundness-sweep pass.
**LESSON ≠ PGN — THE PATTERN (established 2026-06-25):** the Watch beats are the hand-authored, narrationAccuracy-GATED content the student actually sees, so the fix is **align the card `pgn` TO the beats' final move line + rewrite the explanation to match + run the 6-gate set** (variationMiddlegameDepth, narrationAccuracy, lessonTabIntegrity, repertoire-orientation, modelGames-orientation, contentConsistency). Verify divergence first via beats-last-`moves` vs `sanLine`; many "divergences" are just transpositions (same finalFen — leave) or move-order.
- ✅ DONE: scandinavian (Tiviakov g3 + Qa5-Bd2), alekhine (Two Pawns cxd6-fianchetto), two-knights (Traxler 6.Nxf7). 
- ⚠️ DEFERRED — dutch "Hopton Attack Response": misnamed (it's a Leningrad line, not the 2.Bg5 Hopton) AND a separate "Leningrad e5 Break" tab already exists → investigate whether it's a near-DUP before renaming into a collision.
- ⚠️ Sharp-gambit alignments (two-knights Traxler) end student-down-material by design — flag for the engine/soundness-sweep, but the beats are an honest sharp showcase (cf. Muzio/Moller doctrine).
- REMAINING (~10): scotch, dragon Levenfish, evans Lasker, petrov Cochrane, grunfeld (f3), slav, semi-slav, kings-indian, benoni, pirc — same pattern, each its own investigation.

**🔴 (orig.) Biggest remaining structural class — LESSON ≠ PGN (~14 openings):** the variation's Watch/Learn beats teach a DIFFERENT line than the card's stored `pgn`/`sanLine` (scotch, sicilian-dragon Levenfish, evans Lasker, scandinavian, alekhine, petrov Cochrane, dutch Hopton/Leningrad, two-knights Max Lange, grunfeld (f3), slav, semi-slav, kings-indian, benoni Snake). Per opening: decide which line is canonical (beats vs pgn vs DB), align them, then fix the prose. Higher value (student watches one line, plays another) but needs per-opening chess judgment — NOT a quick prose sweep.
Original note:
