...
| 4 | Coach · Review | `/coach/review`, `/coach/review/:gameId`, `/coach/report` | 🔄 in progress | `audit-coach-review-functional.mjs` built. Structure: list → open a game → SUMMARY card → tap **Start** (gated on LLM walk-narration; "Preparing…"→"Start") → move walk. Audit must (a) pick a sample WITH blunders (Morphy) for classification/capture/show-me, (b) wait for Start to ENABLE. 0 console errors so far. Watch: if Start never enables on a sample = a real wedge. |
