...
### Phase D — Model-game overview accuracy  `[DONE 2026-06-26 — mechanical factual errors fixed; remainder stale/soft]`
Re-checked the 206 model-game findings against CURRENT `model-games.json` (the `_findings-revalidated.json` is dated Jun 24, BEFORE the prior student-losing-game fixes — much of it is stale). Verified mechanically by re-deriving from the live records, not trusting the stale judge list:
- **15 real factual errors fixed (shipped, commit 8e6033a):**
  - **5 wrong winner named** — overview credited the WHITE player when the record (`result`+`studentSide`) says the BLACK/student player won: `mg-pirc-czech` (Adams beat Leko — and "Kotronias" wasn't even in the game), `mg-semislav-botvinnik`, `mg-kid-fourpawns`, `mg-grunfeld-russian`, `mg-qid-nimzowitsch`.
  - **10 Hikaru pro-rep wrong years** — overview's `, YYYY` date ≠ the chess.com archive date (e.g. caro-exchange 2020→2023, pirc-nf3 2021→2024). The `(rating)` in parens was correctly left alone.
- **Already-clean classes (0 current bugs):** student-side-LOSING games = 0 (the "Carlsen beats Ding 1-0/studentSide black" + Kasparov findings were fixed in a prior phase; `modelGames-orientation` gate green). Result-claim (win/draw) mismatches = 0 (the 3 hits were "refusing the draw"/"no draw"/"drawish" negations). Boilerplate that SURFACES = 0 (all 5 boilerplate overviews are filtered by `isNarratedModelGame` / have no `studentSide`). `mg-adams-sicilian-alapin` (Deep-Blue field mismatch) no longer exists.
- **1 full-PGN duplicate** (`mg-lichess-BfvmTiCI` == `mg-vienna-nakamura`, Nakamura–Firouzja) is the SAME game under `vienna-game` AND `vienna-gambit` with DISTINCT, opening-tailored overviews — a legit cross-opening reuse (it's a Vienna Gambit, valid for both), NOT a defect. Left.
...
**Per-variation custom first-beat narration `[RESOLVED 2026-06-26 — already satisfied, verified by sweep; no bulk authoring warranted]`:** The intent was to make each walk-in feel tailored to its variation. A full sweep of every lesson's first beat (`scratchpad/sweep-firstbeat-narration.ts` over all 684 via `getAllLessonScripts()`) shows the narration is ALREADY hand-authored and distinctive per variation, and the walk-in change made it land correctly. Findings: of 684, only 25 first beats flagged, and ALL are false positives or intentional —
- **23 "JUMPIN" ("already") = correct, and now BETTER.** They describe real piece placement ("the bishop already eyeing f2", "the b2-bishop already owns the diagonal"). With the silent walk, phrases like the Dragon's "the c-pawn has already been swapped" now refer to the swap the student JUST WATCHED — the walk turned a confusing dropped-in "already" into an accurate just-watched one. Board-accuracy is gated (narrationAccuracy, 5917 tests green).
- **2 "DUP" = intentional reuse.** `pro-hikaru-nimzo-larsen::1...e5 Main Line` deliberately points at the main lesson object (its spine IS the e5 line — see the variations-file comment), so the identical first beat is the same script, not a copy-paste defect.
- **1 "THIN" (stafford-gambit::Nc3, 17 words) = fine** — a specific, vivid opener.

The narrations are vivid and variation-specific by design (the Breyer "features the strangest move in the whole opening"; the Evans "one of the most romantic weapons in all of chess"; the Dragon names the half-open c-file + opposite-wing race). Authoring 684 replacements would be redundant busywork and would risk board-inaccuracy — against "empty > generic > invented; don't fabricate." So this layer is DONE: custom narration already exists and the walk-in now delivers it at the arrival position. Re-open only if a specific opening's opener is found weak in use.
