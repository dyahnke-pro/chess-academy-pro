...
| Punish-gems weapon section + WLPP voice contract (`/openings/:id`) | ✅ | — | [`scripts/audit-punish-gems-loop.mjs`](../scripts/audit-punish-gems-loop.mjs) | `punishGems.test.ts`, `wlppNarration.test.ts` | 2026-05-23. **3-PASS CONTRACT (David): MET only on 3 CONSECUTIVE error-free tiers** — Tier 1 breadth (every gem-bearing opening: card + 4 WLPP buttons + Watch mounts the played-out line + prose fires), Tier 2 failure-modes (cold IDB, no-gem tab self-hide, pick-before-load, out-of-order, Caro named-trap Learn, AND the voice contract decoded off `/api/tts`: Learn=move-dictation-only, Practice=silent), Tier 3 adversarial (wrong move in Learn, rapid toggling, full playout, silence-under-interaction, zero pageerrors). Any error resets the streak. The warmup `.` Polly probe is excluded from the voice checks. Unit gates prove the DATA (gem legality + DB-anchor + the WLPP narration contract: Watch authored prose / Learn moves-only / Practice silent); the loop proves it RENDERS + behaves on the wire. |
