# Streaming action-parsing diagnostic

Working tree: main at `bf9c351c6e430a4556dde44a1c92489813ffea05`

Symptom from April 26 audit log: brain emits `[[ACTION:play_move {"san":"a6"}]]` in streaming responses, the literal action tag leaks into voice output (the user hears the brain say the action syntax), and zero `coach-brain-tool-called: play_move` audits fire — meaning the streaming path is NOT dispatching parsed actions to the toolbelt.

---

## 1. `grep -n "parseActions\|ACTION:\|streaming" src/coach/coachService.ts | head -40`

```
12: *      `[[ACTION:name {args}]]` tag protocol parsed inside the
75:   *  streaming path (if implemented) and pipes raw token chunks
82:   *  non-streaming — intermediate text isn't user-facing. */
181:      summary: `provider=${provider.name} streaming=${useStreaming} trip=${trip}/${maxRoundTrips}`,
```

---

## 2. `cat src/coach/coachService.ts` (full file)
...
## 3. `grep -rn "parseActions" src/coach 2>/dev/null`

```
src/coach/providers/anthropic.ts:18:import { parseActions } from '../../services/coachActionDispatcher';
src/coach/providers/anthropic.ts:56:    const parsed = parseActions(raw);
src/coach/providers/deepseek.ts:18:import { parseActions } from '../../services/coachActionDispatcher';
src/coach/providers/deepseek.ts:30:  const parsed = parseActions(raw);
```

---

## 4. `grep -B2 -A10 "callStreaming\|onChunk" src/coach/coachService.ts src/coach/providers/deepseek.ts 2>/dev/null | head -80`

```ts
src/coach/providers/deepseek.ts-async function callDeepSeek(
src/coach/providers/deepseek.ts-  envelope: AssembledEnvelope,
src/coach/providers/deepseek.ts:  onChunk?: (chunk: string) => void,
src/coach/providers/deepseek.ts-): Promise<ProviderResponse> {
src/coach/providers/deepseek.ts-  const systemPrompt = formatEnvelopeAsSystemPrompt(envelope);
src/coach/providers/deepseek.ts-  const userMessage = formatEnvelopeAsUserMessage(envelope);
src/coach/providers/deepseek.ts-  const promise = getCoachChatResponse(
src/coach/providers/deepseek.ts-    [{ role: 'user', content: userMessage }],
src/coach/providers/deepseek.ts-    systemPrompt,
src/coach/providers/deepseek.ts:    onChunk,
src/coach/providers/deepseek.ts-    'chat_response',
src/coach/providers/deepseek.ts-    2000,
src/coach/providers/deepseek.ts-    'medium',
src/coach/providers/deepseek.ts-  );
src/coach/providers/deepseek.ts-  const timeout = new Promise<string>((_, reject) =>
src/coach/providers/deepseek.ts-    setTimeout(() => reject(new Error('coach-brain-deepseek-timeout')), PROVIDER_TIMEOUT_MS),
src/coach/providers/deepseek.ts-  );
src/coach/providers/deepseek.ts-  let raw: string;
src/coach/providers/deepseek.ts-  try {
src/coach/providers/deepseek.ts-    raw = await Promise.race([promise, timeout]);
--
src/coach/providers/deepseek.ts-    return callDeepSeek(envelope, undefined);
src/coach/providers/deepseek.ts-  },
src/coach/providers/deepseek.ts:  async callStreaming(envelope, onChunk) {
src/coach/providers/deepseek.ts:    return callDeepSeek(envelope, onChunk);
src/coach/providers/deepseek.ts-  },
src/coach/providers/deepseek.ts-};
src/coach/coachService.ts-   *  here as they arrive. The final returned `CoachAnswer.text` is
src/coach/coachService.ts-   *  the post-action-stripped, full response — same semantics as
src/coach/coachService.ts:   *  `runAgentTurn`'s `onChunk`. WO-BRAIN-02.
src/coach/coachService.ts-   *
src/coach/coachService.ts-   *  Streaming only applies to the FIRST turn of a multi-turn loop.
src/coach/coachService.ts-   *  Follow-up turns (when `maxToolRoundTrips > 1`) always run
src/coach/coachService.ts-   *  non-streaming — intermediate text isn't user-facing. */
src/coach/coachService.ts:  onChunk?: (chunk: string) => void;
src/coach/coachService.ts-  /** Maximum number of provider round-trips for tool result loop-back.
src/coach/coachService.ts-   *  Default `1` preserves BRAIN-01..03 single-pass behavior. Move-
src/coach/coachService.ts-   *  selector surface uses `3` so the brain can fetch data via
src/coach/coachService.ts-   *  cerebellum tools, see results, and emit a `play_move` decision in
src/coach/coachService.ts-   *  a follow-up turn. WO-BRAIN-04. */
...
src/coach/coachService.ts-    }
src/coach/coachService.ts-
src/coach/coachService.ts-    // Dispatch tool calls (each gets the surface context).
src/coach/coachService.ts-    const toolResults: { name: string; ok: boolean; result?: unknown; error?: string }[] = [];
src/coach/coachService.ts-    for (const call of lastResponse.toolCalls) {
```

---

## Bonus context — provider interface (so we can see what callStreaming returns and what guarantees it makes about action tags)
