...
vi.mock('../../services/voiceService', () => ({
  voiceService: {
    ...
    speakLecture: vi.fn().mockResolvedValue(undefined),
    stop: vi.fn(),
  },
}));
...
describe('GameChapterPage', () => {
  ...
  it('hint button advances through hint levels and reveals nudge text', async () => {
    ...
    // David 2026-05-26: one tap → the answer. The first press jumps
    // straight to Tier 3 (move arrow + nudge text), no escalation ladder.
    // Wrap in `act` so React flushes the synchronous tier bump AND the
    // async IIFE inside requestHint (the post-await setHintState that
    // lands nudgeText) before the assertions fire.
    await act(async () => {
