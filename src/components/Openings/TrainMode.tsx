...
export function TrainMode({ opening, lines, sectionLabel, onExit }: TrainModeProps): JSX.Element {
  ...
  useBoardContext(currentFen, currentLine.pgn, Math.floor(currentMoveIndex / 2) + 1, opening.color, trainTurn, ctxLastMove, ctxHistory);
  ...
  // ─── Board screen ──────────────────────────────────────────────────────
  return (
    <div className="flex flex-col flex-1 min-h-0 overflow-y-auto pb-[calc(4.5rem+env(safe-area-inset-bottom,0px))] md:pb-0" data-testid="train-mode">
      {/* Top bar */}
