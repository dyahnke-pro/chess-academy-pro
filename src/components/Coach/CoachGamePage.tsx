 ...
 *  on expiry. Matches coachContextEnricher's withFetchTimeout helper.
 *  Distinct from `../../coach/withTimeout` (which returns a
 *  discriminated `{ ok: true | false }` result) — this one is used
 *  by Lichess cloud-eval / explorer fetches that already use
 *  try / catch and want a thrown error on timeout, not a
 *  status-coded value. */
  ...
  useEffect(() => {
    ...
    const makeCoachMove = async (): Promise<void> => {
      ...
      try {
        ...
        // Primary ask = 15 s; Level 1 (no stockfish) = 10 s;
        // Level 2 (no data tools, system addendum) = 8 s;
        // Level 3 = deterministic legal move from chess.js.
        const askInput = {
        ...
        try {
          ...
          if (!primary.ok) {
            ...
            if (!lvl1.ok && !brainPickSan) {
              // Level 2 — pure-LLM, no data tools, system addendum.
              const llmOnlyAsk =
                moveSelectorAsk +
