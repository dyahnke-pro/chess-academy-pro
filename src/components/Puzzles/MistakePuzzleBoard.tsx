...
function extractReplayMoves(pgn: string, playerColor: 'white' | 'black', moveNumber: number): ReplayStep[] {
    ...
    void (async () => {
      ...
      try {
        ...
        if (game?.pgn && !cancelledRef.value) {
          steps = extractReplayMoves(game.pgn, puzzle.playerColor, puzzle.moveNumber);
        }
      } catch {
