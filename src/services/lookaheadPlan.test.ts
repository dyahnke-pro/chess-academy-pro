...
describe('two plans, one line', () => {
  ...
  it('speaks the student\'s plan as "you" and the opponent\'s as "they"', () => {
    ...
    // neither is missing.
    //
    // 🔴 THIS ASSERTED /\bwe\b/ UNTIL 2026-09-21, citing "Naroditsky's
    // collaborative voice, 2026-08-23". That rationale was SUPERSEDED five days
    // later by the locked 2026-08-28 perspective rule — student = "you/your",
    // opponent = "they/their", and we/our/us BANNED as the ambiguity source
    // ("whose piece?"). The old claim is deleted rather than annotated, and the
    // ban is now asserted POSITIVELY below so this test cannot pass on prose
    // that reintroduces it.
    const said = [plan?.theirs.text, plan?.mine.text].filter(Boolean).join(' ');
    expect(said).toMatch(/\byou\b/i);
    expect(said, 'we/our/us is banned (locked 2026-08-28)').not.toMatch(/\b(we|our|us)\b/i);
...
describe('the read sees the whole line, not just where pieces land', () => {
  ...
  it('still speaks the future tense for a mate the line is HEADING toward', () => {
    const base = {
      ...buildSidePlan(), color: 'white' as const, headingFor: [], opening: [], trading: [], outposts: [],
...
describe('the sentence is ordered by the position, not by a fixed ladder', () => {
  ...
  const base: SidePlan = {
    ...buildSidePlan(), color: 'white', headingFor: [], opening: [], trading: [], outposts: [],
    passedPawns: [], materialSwing: 0, shieldStripped: 0, tactic: null,
  ...
  it('says EVERYTHING the line earned, most important first', () => {
    ...
    const lead = loud.replace(/^(?:You|They) want to /, "");
...
describe('hallucination is impossible by construction, not by gate', () => {
  ...
  it('never names a square that was not in the computed input', () => {
    for (let i = 0; i < 2000; i += 1) {
      const plan: SidePlan = {
        ...buildSidePlan(), color: i % 2 ? 'white' : 'black',
  ...
  it('never emits a move, at any combination of facts', () => {
    for (let i = 0; i < 2000; i += 1) {
      const said = describePlan(buildSidePlan({
  ...
  it('is a pure function of its input — same plan, same words, every time', () => {
    ...
    // crept in.
    const plan: SidePlan = {
      ...buildSidePlan(), color: 'white', headingFor: ['e4'], opening: ['c'], trading: ['knight'],
...
describe('the plan does not repeat itself', () => {
  ...
  const P: SidePlan = {
    ...buildSidePlan(), color: 'white', headingFor: ['e4', 'f3'], opening: ['d'], trading: ['knight'],
...
describe('the drift line does not repeat itself', () => {
  ...
  const drift = (): SidePlan => ({
    ...buildSidePlan(), color: 'white', headingFor: ['d4', 'c3'], opening: [], trading: [], outposts: [],
    passedPawns: [], materialSwing: 0, shieldStripped: 0, tactic: null,
...
describe('the rest of the inventory — everything the line leaves behind', () => {
  ...
  it('does NOT list the idle pieces as something a side WANTS', () => {
    ...
    const P: SidePlan = {
      ...buildSidePlan(), color: 'white', headingFor: [], opening: [], trading: [], outposts: ['d5'],
      passedPawns: [], materialSwing: 0, shieldStripped: 0, tactic: null,
...
describe('what the plan leaves out gets its OWN sentence', () => {
  ...
  const planWithIdle = (idle: string[]): SidePlan => ({
    ...buildSidePlan(), color: 'white', headingFor: ['d5'], opening: [], trading: [], outposts: ['d5'],
