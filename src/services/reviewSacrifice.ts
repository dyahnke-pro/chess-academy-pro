...
export function sacrificeCompensation(
  ...
  // 1. Enemy king stuck in the CENTRE — past the opening (fullmove >= 8), the king
  //    still sits on a central file on its own half, and a central file (d/e) is
  //    OPEN, so it's genuinely exposed rather than merely un-castled. Board-true.
  const fullmove = Number(fenAfter.split(' ')[5] ?? '0');
  const enemyKingSq = findKing(board, enemy);
  const centralKing = enemyKingSq !== null
    && ['c', 'd', 'e', 'f'].includes(enemyKingSq[0])
    && (enemy === 'w' ? ['1', '2'].includes(enemyKingSq[1]) : ['7', '8'].includes(enemyKingSq[1]));
  let openCentral = false;
  try {
    const s = describeStructure(fenAfter);
    openCentral = !!s && (s.pawns.openFiles.includes('d') || s.pawns.openFiles.includes('e'));
  } catch { openCentral = false; }
  if (fullmove >= 8 && centralKing && openCentral) {
