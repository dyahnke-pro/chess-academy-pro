...
describe('buildDeliberation — the weighing from the fan', () => {
  ...
  it('honours the "first 3, maybe 4" cap', () => {
    ...
    const d = buildDeliberation({ analysis: wide, fenBefore: FEN, moverColor: 'w', maxCandidates: 3, opponentLastSan: null })!;
  ...
  it('drops the taught move from the weighing (excludeSan — no self-contradiction)', () => {
    ...
    const d = buildDeliberation({ analysis, fenBefore: FEN, moverColor: 'w', opponentLastSan: null, excludeSan: 'd3' })!;
  ...
  it('stays SILENT on a quiet position — coin-flip alternatives are filler, not a fork', () => {
    ...
    const d = buildDeliberation({ analysis: quiet, fenBefore: FEN, moverColor: 'w', opponentLastSan: null })!;
  ...
  it('is NOT a real choice when the fan has one line (forced / only-move)', () => {
    const d = buildDeliberation({ analysis: { topLines: [line(1, 30, 'e1g1')] }, fenBefore: FEN, moverColor: 'w', opponentLastSan: null })!;
...
describe('S5 — a candidate is a lesson only with its reason', () => {
  ...
  it('NEGATIVE CONTROL: a line that proves nothing gets no invented reason', async () => {
    ...
    const fen = 'r1bqkbnr/pppp1ppp/2n5/4p3/2B1P3/5N2/PPPP1PPP/RNBQK2R b KQkq - 3 3';
    const d = buildDeliberation({
      fenBefore: fen, moverColor: 'b', opponentLastSan: null,
      analysis: { topLines: [
        { rank: 1, evaluation: -20, mate: null, moves: ['g8f6'] },
...
describe('an exchange is not "drops the rook" (hand walk 2026-09-24)', () => {
  it('19…Rd8: Rxd8 trades rooks — it drops nothing', () => {
    ...
    const d = buildDeliberation({ analysis: analysis as never, fenBefore: fen, moverColor: 'w', opponentLastSan: null })!;
...
describe('a capture that wins material says so (hand walk 2340, move 9)', () => {
  ...
  it('a plain recapture is a trade, not a win', () => {
    ...
    const d = buildDeliberation({ analysis: { topLines: [line(1, 30, 'e1g1'), line(2, -250, 'f3e5')] }, fenBefore: FEN, moverColor: 'w', opponentLastSan: null })!;
    expect(d.bestWhy).toMatch(/castles/);
  });
});

describe('a recapture is the trade finishing (hand walk 2026-09-25, Qxd8 Rexd8)', () => {
  it('"it wins the queen on d8" is never said of taking back', async () => {
    const { Chess } = await import('chess.js');
    const c = new Chess();
...
describe('mate is the reason (hand walk 1200: "Qxd6# — it wins the bishop on d6"; the spoken SAN already says checkmate)', () => {
