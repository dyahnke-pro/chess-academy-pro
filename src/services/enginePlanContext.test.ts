...
describe('a fresh plan search self-limits', () => {
  ...
  it('uses the budgeted search, never the unbounded one', async () => {
    getCachedStockfish.mockReturnValue(undefined);
    analyzeWithBudget.mockResolvedValue(analysis({ bestMove: 'e2e4', topLines: [{ moves: ['e2e4', 'e7e5'], evaluation: 30, mate: null }] }));
    const plan = await buildEnginePlan(START, 'white');
  ...
  it('passes a real budget, so the search can stop itself', async () => {
    getCachedStockfish.mockReturnValue(undefined);
    analyzeWithBudget.mockResolvedValue(analysis({ bestMove: 'd2d4', topLines: [{ moves: ['d2d4'], evaluation: 20, mate: null }] }));
  ...
  it('honours a caller-supplied budget', async () => {
    getCachedStockfish.mockReturnValue(undefined);
    analyzeWithBudget.mockResolvedValue(analysis({ bestMove: 'd2d4', topLines: [{ moves: ['d2d4'], evaluation: 0, mate: null }] }));
  ...
  it('stamps the position it was computed for', async () => {
    ...
    getCachedStockfish.mockReturnValue(analysis({ bestMove: 'g1f3', topLines: [{ moves: ['g1f3'], evaluation: 15, mate: null }] }));
    const plan = await buildEnginePlan(START, 'white');
