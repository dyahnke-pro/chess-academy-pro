...
import { rankOpeningsByVolume, clearsVolumeFloor, HOME_OPENING_MIN_GAMES, homeOpeningRow } from './openingVolumeFloor';
import type { HomeOpeningChoiceRecord, OpeningKey } from '../types';
...
describe('clearsVolumeFloor', () => {
  it('≥10 games AND ≥5% share clears; 3 of 460 does not; a 5% share of 5 games does not (five games is noise)', () => {
    expect(clearsVolumeFloor(HOME_OPENING_MIN_GAMES, 100)).toBe(true);
    expect(clearsVolumeFloor(HOME_OPENING_MIN_GAMES, COLOUR_GAMES)).toBe(false); // 10 of 460 = 2%: not what they play
    expect(clearsVolumeFloor(63, COLOUR_GAMES)).toBe(true);
    expect(clearsVolumeFloor(3, COLOUR_GAMES)).toBe(false);
    expect(clearsVolumeFloor(5, 100)).toBe(false);  // the share arm never stands alone
    expect(clearsVolumeFloor(12, 400)).toBe(false); // 3% share: not what they play
...
describe('homeOpeningRow — the persisted home opening as a ranked row (A3)', () => {
  it('null without a choice; a choice is never thin and carries the record', () => {
    expect(homeOpeningRow('white', null)).toBeNull();
    expect(homeOpeningRow('black', undefined)).toBeNull();
    const choice: HomeOpeningChoiceRecord = { family: 'Pirc Defense', key: 'b07-pirc-defense' as OpeningKey, games: 63, score: 0.49, source: 'computed', chosenAt: 1 };
    expect(homeOpeningRow('black', choice)).toEqual({ name: 'Pirc Defense', color: 'black', games: 63, winRate: 49, openingId: 'b07-pirc-defense', thin: false, deficit: 63 });
  });
});
