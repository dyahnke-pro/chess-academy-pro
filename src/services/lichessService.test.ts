...
describe('lichessService', () => {
  ...
  it('throws on API error', async () => {
    ...
    await expect(importLichessGames('baduser')).rejects.toThrow('404');
  });
  ...
  describe('multi-game NDJSON parse', () => {
  ...
  describe('malformed NDJSON handling', () => {
    it('throws when a line is not valid JSON', async () => {
      ...
      // The service uses JSON.parse without try/catch, so it throws
  ...
  describe('HTTP error responses', () => {
    it('throws on 404 response', async () => {
      ...
      await expect(importLichessGames('notfound')).rejects.toThrow('Lichess API error: 404');
    });
    ...
    it('throws on 500 server error', async () => {
      ...
      await expect(importLichessGames('servererr')).rejects.toThrow('Lichess API error: 500');
    });
  });

  describe('GameRecord field transformation', () => {
    ...
    it('maps opening ECO code', async () => {
      const ndjson = JSON.stringify(makeLichessGame({
        id: 'eco1',
      ...
      await importLichessGames('user');
      const game = await db.games.get('lichess-eco1');
      expect(game?.eco).toBe('C65');
    });

    it('sets eco to null when opening is missing', async () => {
      const ndjson = JSON.stringify(makeLichessGame({
        id: 'noeco1',
        opening: undefined,
      }));
      ...
      await importLichessGames('user');
      const game = await db.games.get('lichess-noeco1');
      expect(game?.eco).toBeNull();
    });
    ...
    it('converts createdAt timestamp to ISO date string', async () => {
      const ts = new Date('2024-06-15T12:00:00Z').getTime();
      const ndjson = JSON.stringify(makeLichessGame({ id: 'date1', createdAt: ts }));
      vi.spyOn(globalThis, 'fetch').mockResolvedValue({
        ok: true,
        text: () => Promise.resolve(ndjson),
      } as Response);
      ...
      await importLichessGames('user');
      const game = await db.games.get('lichess-date1');
      expect(game?.date).toBe('2024-06-15');
    });
