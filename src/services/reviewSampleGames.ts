...
import { SAMPLE_GAME_ID_PREFIX } from './sampleGames';

// Every fixture id starts with `SAMPLE_GAME_ID_PREFIX` — that prefix is how
// every reader in the app (`isSampleGame`) keeps a fixture out of the
// student's record (D5). `reviewSampleGames.test.ts` proves it for each id.
void SAMPLE_GAME_ID_PREFIX;
...
export const SAMPLE_GAMES: SampleGame[] = [
  {
    ...
    annotations: [
      ...
      { m: 6, c: 'white', san: 'Bc4', eval: 280, classification: 'good', comment: 'Development with tempo — the bishop comes out AND eyes f7 in one move. This is the whole lesson: every piece enters with a purpose.' },
      ...
      { m: 12, c: 'white', san: 'O-O-O', eval: 1100, classification: 'great', comment: 'King safety and the open d-file in one move — the king tucks away and the rook lands pointing straight at Black, whose king is still stuck in the centre.' },
