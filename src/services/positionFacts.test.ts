...
describe('computePositionFacts — the composer', () => {
  ...
  it('names the standing must-defend, board-true', async () => {
    ...
    expect(r.clauses[0]).toMatch(/threatening to win the knight on e5/);
  });
  ...
  it('frames the decision as the OPPONENT’s intent when the opponent is on move', async () => {
    ...
    expect(r.clauses.some((c) => /opponent/i.test(c))).toBe(true);
    expect(r.clauses.some((c) => /this is the moment to slow down/i.test(c))).toBe(false);
  });
