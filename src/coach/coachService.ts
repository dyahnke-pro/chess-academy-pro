...
export interface CoachServiceOptions {
   ...
   *  the coach-turn fallback chain to retry without the tools that
   *  are suspected of hanging (e.g., `stockfish_eval` when the
   *  engine is stuck). The LLM physically does not see excluded
   *  tools in its toolbelt, AND the spine refuses to dispatch them
   *  if the LLM hallucinates a call. WO-COACH-RESILIENCE. */
