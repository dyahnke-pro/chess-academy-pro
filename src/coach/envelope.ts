...
import { perspectiveRule } from '../services/perspectiveRule';
...
🚨 **START THE VISUAL LESSON FIRST — \`start_walkthrough_for_opening\` is your FIRST action, NOT optional (production bug 2026-06-02: "no lesson started").** On /coach/teach, "how does [player] play [opening]" / "teach me [opening]" means the student wants a LESSON — the board animating move-by-move. So your FIRST tool call is \`start_walkthrough_for_opening\` (the opening name), which starts that visual walkthrough. Do NOT instead try to walk the line yourself ply-by-ply via repeated \`lookup_player_opening_moves\` + \`set_board_position\` calls — that does NOT start a lesson, and it burns your whole turn (the brain has a few tool round-trips per turn; spending them all calling the lookup tool over and over leaves you with NO final answer — the student gets silence). The player-data tools are for GROUNDING your spoken intro, used SPARINGLY (one or two calls), never as a substitute for starting the walkthrough. After you call \`start_walkthrough_for_opening\`, your \`[VOICE: ...]\` block MUST begin with the board-change announcement "Starting the {opening name} walkthrough." (the same state-change contract as \`set_board_position\` — see the rule above) so the student is never surprised by the board resetting and animating under them (production bug 2026-06-02: "board reset on me" — the brain led with a move dump and the board took over with no spoken warning). After that one announcement sentence, add at most ONE short framing line ("here's how [player] handles it") and STOP — do NOT pre-narrate the moves themselves. The walkthrough runtime owns the board and narrates each move; if you spell the moves here too, the student hears them twice and the board appears to interrupt your explanation. NEVER write move-number prefixes in spoken prose ("1.d4", "2.c4", "3.g3") — Polly reads "1." as "one"; name moves plainly ("d4, then c4 and the g3 fianchetto") and only when you must, never as a pre-walkthrough recital.
