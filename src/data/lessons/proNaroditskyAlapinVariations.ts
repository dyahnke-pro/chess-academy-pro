...
const D5_OPEN: LessonScript = {
  ...
  beats: [
    ...
    b({
      ...
      say: "exd5 Qxd5 — forced sequence. The queen on d5 looks active but it's exposed to a tempo-gaining Nc3 once you develop. Black scored only 33.7% AGAINST him in this line — the queen-out simplification doesn't equalise as cleanly as it looks.",
    ...
    b({
      ...
      say: "d4 — the move c3 was setting up since move two. Black's c5 is now an isolated soldier; if Black takes with …cxd4 you recapture and have the classical centre. The d4 push doesn't ATTACK anything yet, but it forces Black to commit.",
    ...
    b({
      ...
      say: "Black develops Nf6 attacking nothing in particular. Your Nf3 defends d4 AND prepares Be2/Bc4 — the queen on d5 can't bother you because you haven't put anything on the long diagonal yet. Patience pays off.",
    ...
    b({
      ...
      say: "Be3 supports d4 and adds central pressure. Black's Nc6 hits d4 too; you're balanced. From here Black's options narrow — the cxd4 trade is forced or near-forced, and the Nb5 jump arrives next.",
    ...
    b({
      ...
      say: "Nb5! — the trip pays off. The knight attacks the d4-pawn AND threatens the c7 + d6 outpost squares. Black has to defend d4 or move the queen off d5 (where Nbxd4 next would attack her). Either way you're already winning the opening battle.",
...
const E6_FRENCH: LessonScript = {
  ...
  beats: [
    ...
    b({
      ...
      say: "d4 d5 e5 — the French Advance structure, only now with the c3-c4 lever still available to you later. Black's kingside is cramped because the e5-pawn kills the f6-square for a knight, so Black's natural defender can never reach the kingside. This is the position the c3 setup was building toward.",
    ...
    b({
      ...
      say: "Bd3 — the kingside attacker's bishop. Aims at h7 once Black castles, and supports the eventual e5-e6 break. Note you DO NOT play Bb5 here — different structure than the d5-open line, different bishop deployment.",
    ...
    b({
      ...
      highlights: [{ square: 'd4', color: KEY }, { square: 'c5', color: SOFT }],
      say: "Black plays …cxd4 — the standard French-Advance lever. You're about to recapture and the structure becomes asymmetric: White owns the central pawn duo (d4-e5), Black has only the d5 pawn. The cramping pays off the rest of the game.",
    ...
    b({
      ...
      say: "Re1 — rook to the open e-file, eyes the e6-pawn that's now permanently weak. Combined with Bd3 + Nc3 you have a textbook kingside-attack setup: every piece points at Black's king position once he castles, and the structure says you'll never run out of ideas.",
    ...
    b({
      ...
      say: "After …Bc5 Black tries to coordinate his pieces, but Ng5 lands first. The knight threatens Nxf7 with the bishop hitting h7 behind it — Black has to give up the bishop pair with …Bxg5 or weaken the kingside with …h6, and either choice hands you a permanent structural edge to convert in the middlegame.",
...
const D6_MAINLINE: LessonScript = {
  ...
  beats: [
    ...
    b({
      ...
      say: "d4 immediately — the dream move. Unlike the d5-open line where d4 has to wait, here Black hasn't challenged the centre so you just claim it. The c3-pawn supports d4 for the rest of the game.",
    ...
    b({
      ...
      say: "cxd4 cxd4 — Black trades on d4 hoping for simplification. You recapture with the c-pawn (NOT the queen) because that keeps the central pawn duo + opens the c-file for your rook. Two structural advantages from one trade.",
    ...
    b({
      ...
      say: "Black develops …Nf6 + …g6 — committing to a King's Indian-style fianchetto. You've reached a KID-reversed structure where You have the extra tempo. Every plan that wins for Black in the KID, wins for White here with one move more.",
      sayShort: '…g6 — KID setup, you have +1 tempo.',
    }),
    b({
      id: 'd6-h3',
      ...
      say: "h3 — the 18% pick at this position, the highest of any move. The prophylaxis is critical: stops …Bg4 pinning your Nf3 (which you haven't played yet) AND prepares an eventual g4 kingside expansion. Slow but principled.",
    ...
    b({
      ...
      say: "…Bg7 / Nf3 — both sides complete development. Note the bishop on g7 has nothing to attack: your d4-pawn is solidly defended. Bg7 will just be a defensive piece for the rest of the game unless Black breaks with …e5, which you prevent next.",
    ...
    b({
      ...
      say: "Bd3 and you castle short. The setup is complete: pawns on c3-d4-e4, knights and bishops out, rooks ready for the c-file and d-file. From here the plan is to wait — let Black find a weakness for you. Once he overcommits the kingside or queenside, e4-e5 or d4-d5 opens lines wherever his king ended up.",
...
const G6_DRAGON: LessonScript = {
  ...
  beats: [
    b({
      ...
      moves: 'e4 c5 c3 g6',
      highlights: [{ square: 'g6', color: KEY }, { square: 'g7', color: SOFT }],
      say: "…g6 — the Hyper-Accelerated Dragon. Black wants the fianchetto setup without committing to …d6 first, hoping to keep his Dragon theory available later. The Alapin's whole point is that you don't need to know his theory: c3 supports d4, the pieces find their squares the same way regardless of move order, and you head for practical positions instead of memorisation. The flexibility he's preserving doesn't matter against a system.",
    ...
    b({
      ...
      say: "d4 cxd4 cxd4 …d5! — Black surprises with a Scandinavian-like central strike. The idea: equalise quickly by trading the d-pawn. You respond exd5 just like in the d5-open line, then transpose into a Panov-style structure.",
    ...
    b({
      ...
      say: "exd5 / Nf6 / Nc3 / Nxd5 — the knight on d5 will be chased shortly. You've reached a Panov-Botvinnik style position where d4 is isolated but you have piece activity. This is a fork in his game: 14% pick Nf3, 12% pick Nc3 first.",
    ...
    b({
      ...
      say: "Bc4! — hit the knight on d5 AND eye f7. Same theme as the main spine even though you got here via a totally different move order. Black has to move the knight, and Nb6 (the natural retreat) trades into your hand.",
...
const NC6_LINE: LessonScript = {
  ...
  beats: [
    b({
      ...
      say: "…Nc6 — Black develops a knight without committing the centre yet. This is a common practical try: keep flexibility, see what White does. The Alapin's answer is that the c3 setup runs the same way against ANY second move — solid yet flexible while remaining surprisingly aggressive once the centre opens. You just play d4 immediately and Black's wait-and-see commitment hands you tempi he can never recover.",
    ...
    b({
      ...
      say: "d4 / …cxd4 / cxd4 / …d5 / exd5 / …Qxd5 — Black tries the same Scandinavian-style simplification as in the d5-open line, but via a different move order. You end up in a similar structure, except now Black has committed the Nc6 — which limits his options later.",
    ...
    b({
      ...
      say: "…Qd8 — the only safe square. Black's queen is now exactly where it started, but White has knights on c3 and f3 and the c-file is open. Two tempi lost out of the gate and the structural disadvantage will carry the whole game.",
    ...
    b({
      ...
      say: "Bd3 / Nf6 / O-O / Be7 / a3 — you develop classically and prepare b4 expansion. The d4-pawn looks isolated but Black's c8-bishop is locked in by his own e6-pawn, so he can't pressure it. White converts the structural edge in the endgame, where the bishop pair and the queenside space carry the rest of the way.",
...
const NF6_D6_SUB: LessonScript = {
  ...
  beats: [
    ...
    b({
      ...
      highlights: [{ square: 'c4', color: KEY }, { square: 'd5', color: ATK }],
      say: "Bc4! — hit the knight on d5 AND aim at f7. Black expected dxe5 transposition into safe territory; you ignore the e5-pawn entirely and develop attacking. The whole gambit complex rests on Black mishandling the tactical sequence.",
    ...
    b({
      ...
      say: "After …Nb6 the e-pawn pushes forward — e6!! The pawn touches f7 and Black has to react. This is the gambit's signature: you sacrifice the bishop or pawn to crack open the king position before Black can develop.",
    ...
    b({
      ...
      say: "…Nxc4 grabs the bishop, but you play Qa4+! — the check that wins material back. Black has to block with Nc6 (the only reasonable interposition), and now the position is wide open.",
    ...
    b({
      ...
      say: "After …d5, your queen checks again with Qf4+, the king runs to g8, and now d4 opens the centre. Black's king sits on g8 with no f7-pawn, the e7-pawn falls next, and every white piece floods the board through the open lines. The whole point of the exf7+ sacrifice was to reach exactly this position with development on your side.",
    ...
    b({
      ...
      say: "Rd1 / dxc5 / c4 — White's pawn duo on c4-d4 takes the central squares and the rook on d1 pins down Black's coordination. Once the position reaches this point Black needs to find a string of precise defensive moves with the king on g8 and no shelter, and that almost never happens in practice. The whole gambit converts on Black's first imprecision.",
...
const NF6_E6_SUB: LessonScript = {
  ...
  beats: [
    b({
      ...
      highlights: [{ square: 'e6', color: KEY }, { square: 'd5', color: SOFT }],
      say: "Inside the …Nf6 spine — Black's quieter …e6 setup. Black defends the d5-knight with e6 and aims for solid development, refusing the gambit complications of …d6. You respond with d4 and the position transposes into Panov-Botvinnik-style IQP territory. The framework holds: logical piece development on your side, central control, and a practical middlegame where Black's solidity meets White's activity over the long haul.",
    ...
    b({
      ...
      say: "…Bd7 / Rd1 — Black develops the queen's bishop, you put the rook on the d-file supporting d4 AND preparing d5 break. The structure favours White because EVERY plan (d5 push, Bg5 pin, Ne5 outpost) increases White's pressure while Black's plans (…b5, …Bc6, …Nb6) just shuffle pieces.",
    ...
    b({
      ...
      highlights: [{ square: 'c3', color: KEY }, { square: 'd5', color: ATK }],
      say: "…Bc6 / Nc3 — Black redirects the bishop to c6, you develop the queen knight finally hitting the d5-knight. The whole position rests on the d5 exchange that's now coming: whoever wins that trade controls the central squares for the rest of the middlegame.",
    ...
    b({
      ...
      say: "…Nxc3 / bxc3 — the knight trade gives you doubled c-pawns, but they aren't a real weakness here. You get the open b-file for the rook AND the bishop pair AND a reinforced d4 centre. Black trades his best piece for a structural concession he can't punish, and the endgame steers cleanly in White's favour.",
