...
const MAIN: LessonScript = {
  ...
  beats: [
    b({ id: 'open', moves: 'e4 c5 Nc3 Nc6 Bb5 Nd4 Nf3',
      ...
      say: "The Nc3 move order — another way this repertoire dodges Open Sicilian theory. You develop the knight, lean on c6 with Bb5, and when Black leaps into d4 to muddy things, you calmly question the knight with Nf3. No panic, no theory — just sensible developing moves.",
    ...
    b({ id: 'mg-qxd4', moves: 'e4 c5 Nc3 Nc6 Bb5 Nd4 Nf3 Nxb5 Nxb5 a6 Nc3 d6 d4 cxd4 Qxd4',
      ...
      say: "You recapture on d4 with the queen, planting it in the centre with a glance toward g7. Here's the honest middlegame: it's a SMALL edge. You have more space and the better-placed pieces; Black has the bishop pair but no targets for it yet. You castle, develop the dark-squared bishop, and squeeze. Against a Sicilian player who wanted chaos, a quiet, better game is exactly the win you're after.",
...
const VS_D6: LessonScript = {
  ...
  beats: [
    b({
      ...
      say:
        "The …d6 order — seventy-three of his corpus games. You open the centre anyway and recapture with the QUEEN, and when the knight kicks it, the retreat to d2 is the system's signature: the queen sits behind the future long castle, already on its attacking diagonal.",
    ...
    b({
      ...
      say:
        "Black stakes e5, you hold the structure with g3, and the king tucks to b1 — the quiet insurance move every long-castled player pays. The race position is set: his pawns will roll at your king, yours at his, and the corpus says your tempo head-start decides. The middlegame plan maps the tactics from exactly here.",
...
const VS_E6: LessonScript = {
  ...
  beats: [
    ...
    b({
      ...
      say:
        "Black pins with …Bb4 — and you castle long straight through it, the rook arriving on d1 with tempo down the file your queen just left. Move seven, and the whole system stands: queen, bishop, castle, half-open d-file. The middlegame plan picks up with Black's …e5 break and the bishop's return ticket.",
...
const VS_G6: LessonScript = {
  ...
  beats: [
    b({
      ...
      say:
        "The …g6 fianchetto announces where Black's bishop is going — so you trade on c6 before it ever gets there. Look at the wreckage: doubled c-pawns, and when the fianchetto finishes, its long diagonal will stare into Black's own pawn on c6.",
    ...
    b({
      ...
      say:
        "You set the structure with d3 and develop the bishop to e3 — pointed straight at the front twin on c5. The plan is not a storm; it is a collection: those doubled pawns are an annuity, and every developing move adds interest.",
    ...
    b({
      ...
      say:
        "Black spends the front twin at once — …c4 hits your d3-pawn — and after the quiet h3 insurance, the trade comes and you recapture TOWARD the centre. Count the result: your pawn chain rebuilt itself, and where Black once had two c-pawns, one stands alone. A small, permanent, teachable edge — the middlegame plan collects it from here.",
