...
const FIVE_KNIGHTS: LessonScript = {
  ...
  beats: [
    ...
    b({
      ...
      say: "This is the picture to remember: the queen on d6 and the rook on the h-file both point at h2, White's pawns are wrecked, and the king cannot find a safe home. Rosen converted this exact structure against a 2787. The pawn was never the point — the king is.",
...
const SPACE_GRAB: LessonScript = {
  ...
  beats: [
    ...
    b({
      ...
      say: "When White finally develops with Nc3, the bishop pins it from b4. Every piece is in the attack, the e5-pawn is falling, and White still has not castled. Be honest: this line scores a touch lower for Rosen, so play it for the initiative, not for a guaranteed win — but the practical pressure is real.",
...
const KNIGHT_RETREAT: LessonScript = {
  ...
  beats: [
    ...
    b({
      ...
      say: "We pin with Bb4, both sides castle, and the position is dead level with chances for both. The lesson here is restraint: when White declines the gambit correctly, do not force the attack — just develop, equalise, and outplay them later. Rosen beat a 2796 from exactly this calm structure.",
...
const FOUR_KNIGHTS: LessonScript = {
  ...
  beats: [
    b({
      ...
      say: "The cleanest way for White to dodge the gambit is to never take on e5 at all — Nc3, the Four Knights. We do not copy with the symmetric knight move; we play in Stafford spirit and develop the bishop straight toward f2. This is Rosen's highest-scoring line of the whole complex.",
