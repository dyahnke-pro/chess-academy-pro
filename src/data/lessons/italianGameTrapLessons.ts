...
export const ITALIAN_GAME_TRAP_LESSONS: Record<string, LessonScript> = {
  ...
  beats: [
    ...
    b({ id: "l2", moves: "e4 e5 Nf3 Nc6 Bc4 d6 Nc3 Bg4 h3 Bh5", say: "h3 asks the bishop a question. If Black breaks the pin themselves with Bxf3, White simply recaptures and the danger passes. But the greedy, natural human move is Bh5, maintaining the pin and refusing to give up the bishop. That move walks straight into the oldest trick in the book.", sayShort: "h3 Bh5 — Black keeps the pin.", highlights: [H("h5", KEY)] }),
    b({ id: "l3", moves: "e4 e5 Nf3 Nc6 Bc4 d6 Nc3 Bg4 h3 Bh5 Nxe5", say: "Nxe5!! The pinned knight moves anyway — and grabs a pawn. It looks like a blunder hanging the queen, and if Black sees the whole picture they play Nxe5 declining the queen, staying only a pawn down. But the bait is irresistible: the white queen on d1 sits there for the taking. Most opponents cannot resist.", sayShort: "Nxe5!! — the pinned knight moves and grabs e5.", highlights: [H("e5", KEY), H("d1", SOFT)] }),
    b({ id: "l4", moves: "e4 e5 Nf3 Nc6 Bc4 d6 Nc3 Bg4 h3 Bh5 Nxe5 Bxd1", say: "Bxd1?? Black snatches the queen, certain they have won. They have, in fact, just lost — by force, in two moves. White doesn't recapture the bishop. White plays for mate, and the f7-square, weak since move three, is the door.", sayShort: "…Bxd1?? — the queen grab that loses by force.", highlights: [H("d1", KEY), H("f7", SOFT)] }),
  ...
  beats: [
    ...
    b({ id: "f4", moves: "e4 e5 Nf3 Nc6 Bc4 Nf6 Ng5 d5 exd5 Nxd5 Nxf7 Kxf7 Qf3+", say: "Nxf7! Kxf7 Qf3+ — the knight sacrifices itself on f7, dragging the king out into the open on the seventh rank, and the queen swings to f3 with check. From f3 she does two jobs at once: she checks the exposed king and she skewers the loose d5-knight down the diagonal. Black is up a knight, but their king is walking and their pieces are hanging.", sayShort: "Nxf7! Kxf7 Qf3+ — drag the king, hit d5.", arrows: [A("f3", "f7", ATK), A("f3", "d5", ATK)], highlights: [H("f7", KEY), H("d5", SOFT)] }),
    b({ id: "f5", moves: "e4 e5 Nf3 Nc6 Bc4 Nf6 Ng5 d5 exd5 Nxd5 Nxf7 Kxf7 Qf3+ Ke6 Nc3", say: "Ke6 — the only move. The king must step forward to e6 to defend the pinned d5-knight, because nothing else holds it. And now Nc3 brings the last piece into the assault, piling a third attacker onto the d5-knight and threatening to rip the centre open with the king sitting in the middle of the board. White has sacrificed a knight to lure the king to e6 and aim their whole army at it — at club level this attack is simply winning. The Fried Liver is the reason every Two Knights player must know Na5.", sayShort: "Ke6 Nc3 — the king is hunted.", arrows: [A("c3", "d5", ATK)], highlights: [H("e6", KEY), H("d5", KEY)] }),
  ],
},
