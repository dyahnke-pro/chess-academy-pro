...
const BERLIN: LessonScript = {
  ...
  beats: [
    b({
      ...
      say: "You'll reach the Berlin when Black answers Bb5 with Nf6 instead of a6 — the rock-solid defence that helped Kramnik dethrone Kasparov. Black immediately eyes our e4-pawn and heads for simplification. We do not fear it: the resulting endgame gives White a small, safe, nagging pull, and that is exactly what we play for.",
      sayShort: 'Nf6 — the solid Berlin.',
    }),
    b({
      id: 'oo-nxe4', moves: `${RUY} Nf6 O-O Nxe4`,
      highlights: [{ square: 'e4', color: KEY }],
      say: "We castle, offering the e4-pawn, and Black takes it with Nxe4 — the whole point of the Berlin. Nothing reckless here on either side: grabbing e4 is known theory, and now we open the centre to exploit Black's slightly exposed knight and our lead in development.",
    ...
    b({
      ...
      say: "d4 strikes the centre and hits e5, and the knight retreats to d6, attacking our b5-bishop as it goes. Every move here is forcing and well-charted — we are being carried down a narrow, well-lit corridor toward the famous Berlin endgame.",
      sayShort: 'd4 Nd6 — open the centre.',
    }),
    b({
      id: 'bxc6', moves: `${RUY} Nf6 O-O Nxe4 d4 Nd6 Bxc6 dxc6`,
      highlights: [{ square: 'c6', color: KEY }],
      say: "We trade on c6, and Black recaptures with dxc6 — saddling themselves with doubled c-pawns. That damaged structure is the concession we play against for the rest of the game. In return Black gets the bishop pair, so it is a genuine imbalance: our better pawns versus their two bishops.",
...
const CLOSED_D3: LessonScript = {
  ...
  beats: [
    b({
      ...
      say: "You'll reach this when Black plays the main Closed Ruy — a6, Nf6, Be7, heading for the deeply-analysed lines and, often, the dangerous Marshall Gambit. We castle and prepare a modern, practical decision that sidesteps all of Black's preparation.",
      sayShort: 'The main Closed Ruy setup.',
    }),
    b({
      id: 'd3', moves: `${RUY} a6 Ba4 Nf6 O-O Be7 d3`,
      highlights: [{ square: 'd3', color: KEY }],
      say: "d3 — the modern choice, and a smart one. The old main line plays c3 and d4, but that invites the Marshall Gambit, where Black sacrifices a pawn for a ferocious prepared attack. By keeping the pawn modestly on d3 we deny Black the Marshall entirely and steer into a slow, rich manoeuvring game on our own terms.",
    ...
    b({
      ...
      say: "Black gains queenside space with b5 and we retreat the bishop to b3 — its ideal diagonal, aiming toward f7. Nothing is forced now; this is a battle of slow plans, exactly where a well-prepared player outplays an opponent who only knew the sharp Marshall lines.",
...
// ============================================================
// Steinitz Defense — the old ...d6; we grab the centre with d4.
// ============================================================
const STEINITZ: LessonScript = {
  ...
  beats: [
    b({
      ...
      say: "You'll see this when Black plays the old-fashioned d6 — the Modern Steinitz. It is solid and safe, but passive: Black shores up the e5-pawn at the cost of shutting in the dark-squared bishop and ceding us the centre. Against a passive setup, the right response is to seize space energetically.",
    ...
    b({
      ...
      say: "Black develops the bishop to d7 and we strike with d4 — the point of the whole line. Now we own a strong pawn centre with e4 and d4 against Black's passive d6, and our extra space translates directly into freer, more active pieces. A comfortable structural advantage.",
