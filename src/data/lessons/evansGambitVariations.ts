...
export const EVANS_GAMBIT_VARIATION_LESSONS: Record<string, LessonScript> = {
  "evans-gambit::Evans Gambit: Anderssen Variation": {
    ...
    beats: [
      ...
      b({ id: 'an3', moves: 'e4 e5 Nf3 Nc6 Bc4 Bc5 b4 Bxb4 c3 Ba5 d4 exd4 O-O Bb6 cxd4 d6 Nc3 Nf6 e5 dxe5 Ba3', say: "Nc3 develops with tempo, and White strikes again with e5! — gaining space and hounding the f6-knight. Now the move that makes the whole line: when Black takes on e5, do NOT recapture — the automatic dxe5 lets the queens come off on d1 and the advantage evaporates with them. Instead Ba3!, the classic Evans lunge: the bishop seizes the a3-f8 diagonal and the Black king loses the right to castle for the rest of the game.", sayShort: 'Ba3 — deny the castle, keep the queens.', arrows: [A('a3', 'f8', ATK)], highlights: [H('a3', SOFT), H('f8')] }),
      b({ id: 'an4', moves: 'e4 e5 Nf3 Nc6 Bc4 Bc5 b4 Bxb4 c3 Ba5 d4 exd4 O-O Bb6 cxd4 d6 Nc3 Nf6 e5 dxe5 Ba3 Nxd4 Nxd4 Bxd4 Nb5 Be6', say: "Black grabs on d4 and the punishment is immediate: the knight recaptures, and when the bishop takes it, Nb5! — hitting that d4-bishop while eyeing the c7 hop against king and rook. Black must scramble with Be6, and count the wreckage: the king is stranded on e8 for good, every White piece is active, and the engine reads better than two pawns of advantage. That is the Evans at its best — the gambit pawn is a distant memory, the initiative is everything.", sayShort: 'Nb5 — hit the bishop, eye c7.', arrows: [A('b5', 'd4', ATK), A('b5', 'c7', ATK)], highlights: [H('d4', SOFT), H('c7'), H('e8')] }),
    ],
  },
  ...
  "evans-gambit::Evans Gambit: Stone-Ware Variation": {
    ...
    sources: ['book:evans-gambit', 'concept:pos-center', 'https://en.wikipedia.org/wiki/Evans_Gambit'],
    beats: [
      b({ id: 'sw1', moves: 'e4 e5 Nf3 Nc6 Bc4 Bc5 b4 Bxb4 c3 Bd6', say: "The Stone-Ware — Black declines the sharp retreats and tucks the bishop on …Bd6, planning to return the pawn for a solid, simplified game. White is happy to oblige on their own terms: the recommendation is to play the centre and development, not chase complications, and convert the gambit into a lasting positional pull.", sayShort: '…Bd6 — Black plays it solid; White builds.', highlights: [H('d6', SOFT)] }),
  ...
  orientation: "white",
  beats: [
    b({ id: "d1", moves: "e4 e5 Nf3 Nc6 Bc4 Bc5 b4 Bb6", say: "When Black declines with ...Bb6, refusing the b4-pawn, White doesn't win the big attack for free — but they don't lose anything either. They keep the b4-pawn's gain of space and prepares a4, which will gain still more queenside room and start to harass the b6-bishop.", sayShort: "…Bb6 — Black declines the gambit.", highlights: [H("b6", KEY), H("b4", SOFT)] }),
