...
export const KINGS_INDIAN_ATTACK_VARIATION_LESSONS: Record<string, LessonScript> = {
  ...
  "kings-indian-attack::KIA: e5 Wedge System": {
    ...
    beats: [
      ...
      { id: "kw3", moves: ["e4", "e6", "d3", "d5", "Nd2", "Nf6", "Ngf3", "Nc6", "g3", "dxe4", "dxe4", "Bc5", "Bg2", "O-O", "O-O", "e5", "c3", "a5", "Qc2"], say: "…e5! — Black plants the wedge themselves, freezing White's e4-pawn and claiming their share of the centre. c3 blunts the c5-bishop's diagonal, …a5 grabs queenside space, and Qc2 props e4 from behind. Patience: the wedge that cramps you is also a target.", sayShort: "…e5 — the wedge lands; c3 blunts.", highlights: [{ square: "e5", color: "rgba(255,214,0,0.88)" }, { square: "c3", color: "rgba(80,140,255,0.32)" }] },
  ...
  "kings-indian-attack::KIA vs Sicilian Structure": {
    openingId: "kings-indian-attack", title: "KIA vs the Sicilian setup — a4, then Nc4", minutes: 8, orientation: "white",
    sources: ["concept:pos-outpost", "concept:pos-prophylaxis", "https://en.wikipedia.org/wiki/King%27s_Indian_Attack"],
    beats: [
      { id: "ks1", moves: ["Nf3", "c5", "g3", "Nc6", "Bg2", "g6", "O-O", "Bg7"], say: "Black answers the KIA with a mirror: …c5, …Nc6, the fianchetto. Symmetrical setups punish the impatient — White completes the system first and lets the plans diverge on their terms.", sayShort: "Bg7 — mirrors reward patience.", highlights: [{ square: "c5", color: "rgba(80,140,255,0.32)" }] },
      { id: "ks2", moves: ["Nf3", "c5", "g3", "Nc6", "Bg2", "g6", "O-O", "Bg7", "d3", "d6", "e4", "Nf6", "Nbd2", "O-O"], say: "d3, e4, Nbd2 — the KIA shell, identical against everything, which is exactly its value: while Black chooses among plans, White's pieces already know their squares.", sayShort: "e4 — the shell, same as always.", highlights: [{ square: "e4", color: "rgba(255,214,0,0.88)" }, { square: "d2", color: "rgba(80,140,255,0.32)" }] },
      { id: "ks3", moves: ["Nf3", "c5", "g3", "Nc6", "Bg2", "g6", "O-O", "Bg7", "d3", "d6", "e4", "Nf6", "Nbd2", "O-O", "a4", "Rb8"], say: "…Rb8 telegraphs …b5, the standard Sicilian wing plan — and a4! cancels it before it starts. One prophylactic pawn move, and Black's main idea needs rebuilding from scratch.", sayShort: "a4 — cancel …b5 before it exists.", highlights: [{ square: "a4", color: "rgba(255,214,0,0.88)" }, { square: "b5", color: "rgba(80,140,255,0.32)" }] },
      { id: "ks4", moves: ["Nf3", "c5", "g3", "Nc6", "Bg2", "g6", "O-O", "Bg7", "d3", "d6", "e4", "Nf6", "Nbd2", "O-O", "a4", "Rb8", "Nc4", "b6", "Re1", "Bb7", "c3", "a6"], say: "Nc4! — the d2-knight finds the square this structure hides for it, leaning on d6 and b6 and eyeing the e5-jump. Re1 backs the e-pawn, c3 blunts the g7-bishop's diagonal. The engine calls it dead level — and level with a plan beats level without one. The KIA against the Sicilian: a4 first, knight to c4, then pick your wing.", sayShort: "Nc4 — the knight finds its square.", arrows: [{ from: "c4", to: "d6", color: "rgba(40,185,95,0.92)" }, { from: "c4", to: "b6", color: "rgba(40,185,95,0.92)" }], highlights: [{ square: "c4", color: "rgba(255,214,0,0.88)" }] },
    ],
  },

  "kings-indian-attack::KIA vs Caro-Kann Structure": {
    openingId: "kings-indian-attack", title: "KIA vs the Caro setup — absorb the big centre", minutes: 8, orientation: "white",
    sources: ["concept:pos-center", "concept:pawn-fianchetto", "https://en.wikipedia.org/wiki/King%27s_Indian_Attack"],
    beats: [
      { id: "kc1", moves: ["e4", "c6", "d3", "d5", "Nd2", "e5"], say: "Against the Caro move order the KIA stays the course: d3 and Nd2 absorb …d5, and when Black claims the full centre with …e5, White doesn't argue — the fianchetto bishop will bite that centre for the rest of the game.", sayShort: "…e5 — let him have it, for now.", highlights: [{ square: "e5", color: "rgba(80,140,255,0.32)" }, { square: "d5", color: "rgba(80,140,255,0.32)" }] },
      { id: "kc2", moves: ["e4", "c6", "d3", "d5", "Nd2", "e5", "Ngf3", "Bd6", "g3", "Nf6", "Bg2", "O-O"], say: "…Bd6, …Nf6 — Black's centre looks imposing. White answers with the same three moves as always: g3, Bg2, castle. The g2-bishop stares through d5 at the queenside; the e4/d3 duo waits for the right moment to open the position.", sayShort: "Bg2 — the stare through d5 begins.", highlights: [{ square: "d5", color: "rgba(255,214,0,0.88)" }] },
      { id: "kc3", moves: ["e4", "c6", "d3", "d5", "Nd2", "e5", "Ngf3", "Bd6", "g3", "Nf6", "Bg2", "O-O", "O-O", "Re8", "Re1", "Nbd7"], say: "Re1 meets …Re8 — both rooks load the e-file, and the central tension becomes the whole game. …Nbd7 keeps the e5-pawn triple-guarded. Whoever releases the tension first usually concedes something; the KIA player has learned to wait.", sayShort: "Re1 — load the file, keep waiting.", highlights: [{ square: "e5", color: "rgba(80,140,255,0.32)" }, { square: "e1", color: "rgba(255,214,0,0.88)" }] },
      { id: "kc4", moves: ["e4", "c6", "d3", "d5", "Nd2", "e5", "Ngf3", "Bd6", "g3", "Nf6", "Bg2", "O-O", "O-O", "Re8", "Re1", "Nbd7", "c3", "a5", "Qc2", "dxe4", "dxe4", "Nc5"], say: "c3 and Qc2 improve quietly — and Black blinks: …dxe4 dxe4 releases the centre, and suddenly the g2-bishop's diagonal runs unobstructed while White's e4-pawn holds the middle. …Nc5 hits it immediately. The engine calls it balanced with the structure favouring White's pieces: the Caro built the big centre, and the KIA made letting it go feel necessary.", sayShort: "dxe4 — he blinked; the bishop breathes.", arrows: [{ from: "c5", to: "e4", color: "rgba(40,185,95,0.92)" }], highlights: [{ square: "e4", color: "rgba(255,214,0,0.88)" }] },
    ],
  },
};
