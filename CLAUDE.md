...
4. **Run the audit script(s) for every surface you touched.**
   ...
   | the concept SPOKEN during live gameplay (Learn walkthrough per-ply narration, `landedTacticTeaching` splice, `computePlyFacts.tacticLanded`, the tactic classifier `detectTacticType`) | `scripts/audit-concept-gameplay-prod.mjs` (muted, 3-instrument: asks Learn for a lesson whose taught spine LANDS a tactic — "Torre Attack", ply-5 Bg5 pin — and proves the narration listener heard the engine's invariant sentence spoken mid-lesson; off-canonical ask too) + `npx vitest run src/services/tacticTypeUnification.test.ts src/hooks/usePhaseNarration.test.ts src/hooks/useLiveCoach.test.tsx src/hooks/usePositionNarration.test.ts` |
